...
for i = 2:n_pa
    ...
    [EQi, Qi] = EQ(thetaR, thetaF, varR, varF, true);
    
    Qs{i + 1} = Qi(:, 2);
    deltas{i + 1} = EQi(:, 2) ./ Qi(:, 2);
end
...
for i = 1:numel(Qs)
    ...
    epsilon = 1-CIp;
    lower = betainv(epsilon/2,alphas,betas);    % Upper bound of confidence interval
    upper = betainv(1-epsilon/2,alphas,betas);  % Lower bound of confidence interval
    ...
    text(max(dABs) * 1.01, E_rho(end), lineLabel, 'Interpreter', 'latex', 'FontSize', FontSize);
end
...
if (loadData)
    ...
    for i = 1:length(dABs)
        for j = 1:n_pa
            ...
            % Run simulation
            [~, D0, D1, l, a, b, x] = simulate(N, thetas{:});
