% experiment8_full_hmm: The fully Bayesian approach for simulating a session (HMM) 
...
% File path for loading/saving results (can be empty if not used)
file_path = 'saved_data/experiment8_full_hmm'; 
...
sigma = 2 * noise; % Standard deviation of random variables

% Optimization options (setting to 0 initializes to prior means)
maxEpochs = 0;     % Maximum epochs for outer optimization
maxIters  = 0;     % Maximum iterations for inner optimization

% Sampling options
Ns = 1e4;          % Number of samples
Nb = 1e3;          % Number of burn-in samples
display = false;   % Disable live histogram updates for faster processing
method = 'slice';  % Sampling methods:
...
% Display posterior distributions of the parameters
