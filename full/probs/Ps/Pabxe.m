function [Ps, d_thetaR] = Pabxe(thetaR, thetaF, varR, varF, a, b, x, e)
    ...
    %       for given system parameters, some treated as fixed (thetaF) and others 
    %       as random (thetaR). It returns detection probabilities for four possible 
    %       outcomes (00, 01, 10, 11) and calculates partial derivatives with 
    %       respect to random variables in varR. The pulse parameters (a, b, x, e) 
    %       define Alice and Bob’s basis choices, Alice’s bit choice, and whether 
    %       Eve intercepted the pulse.
    %
    % Inputs:
    ...
    % Beam-splitter probabilities and their derivatives
    [p0, dp0_pe] = p_bs(0, x, a, b, pe);
    p1 = 1 - p0;
    dp1_pe = -dp0_pe;

    Nl = size(lambdas, 2);  % Number of lambda values

    % Complementary probabilities for dark counts
    qd0 = 1 - pd0;
    qd1 = 1 - pd1;

    % Adjust lambda based on eavesdropping probabilities
    hat_lambda = lambdas .* pAB.^(1 - e) .* pAE.^e;

    % Mixed probabilities for pc
    pc_mix = pc1 - p0 .* (pc1 - pc0);

    % Adjusted pc and pd based on eavesdropping
    ...
    if nargout > 1
        ...
        % Iterate over the three cases (or, x1, 1x)
        for i = 1:3
            ...
            % Compute partial derivatives for lambdas
            if outs(2)
            ...
            % Compute partial derivatives for pc
            if outs(3)
            ...
            % Compute partial derivatives for pd
            if outs(4)
            ...
            % Compute partial derivatives for k
            if outs(5)
        ...
        % Adjust d_thetaR for combined probabilities
    ...
    % Combine probabilities into final outputs
    P_or = Ps{1};
    P_x1 = Ps{2};
    P_1x = Ps{3};
    ...
    % Return probabilities divided by number of lambdas
