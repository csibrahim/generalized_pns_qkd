function [Ps, d_thetaR] = P_hmm(thetaR, thetaF, varR, varF)
    ...
    if nargout > 1
        ...
        d_thetaR = varargout{2};  % Transition matrix derivatives w.r.t. thetaE
        m = numel(d_thetaR);      % Number of parameters in thetaR
        
        % Solve the linear system (I-Ts') * dv/dθ = d(Ts')/dθ * v
        A = eye(Nt) - Ts';
        b = cellfun(@(M) M' * v_norm, d_thetaR, 'UniformOutput', false);
        b = cell2mat(b);
        
        % Forcing the constraint sum(v) = 1
        ...
        % Compute derivatives of detection probabilities w.r.t. thetaR
        for i = 1:m
