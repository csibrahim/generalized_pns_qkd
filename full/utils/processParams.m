function [thetaP, thetaR, thetaF, varR, varF] = processParams(thetas, true_thetas, varF, varR, sigma)
    ...
    for i = 1:numel(varR)
        ...
        switch distType(varName)
            case 'gamma'
                [alpha_param, beta_param, ub_param, lb_param] = gamma_parameters(mu, mu * sigma);
            case 'beta'
                [alpha_param, beta_param, ub_param, lb_param] = beta_parameters(mu, mu * sigma);
            case 'eve'
                switch varName
                    case 'dAE'
                    ...
                    case 'k'
                        alpha_param = 1; beta_param = 2 / (max_k - 1); ub_param = inf; lb_param = 1;
                    case 'Delta'
